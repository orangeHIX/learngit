    ...
    private RealMatrix createRatingMatrix(KeyIndex userIndex, KeyIndex itemIndex) {
        ...
        try (ObjectStream<Rating> ratings = dao.query(Rating.class)
            ...
            for(Rating r : ratings) {
                if( userIndex.containsKey(r.getUserId()) && itemIndex.containsKey(r.getItemId())) {
                    int ui = userIndex.getIndex(r.getUserId());
                    int ii = itemIndex.getIndex(r.getItemId());
                    matrix.setEntry(ui, ii,
                            r.getValue()
                                    - baseline.getItemBias(r.getItemId())
                                    - baseline.getUserBias(r.getUserId())
                                    - baseline.getIntercept());     //item bias
                }
            }
        }
