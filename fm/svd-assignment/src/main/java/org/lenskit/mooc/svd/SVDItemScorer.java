...
public class SVDItemScorer extends AbstractItemScorer {
    ...
    public ResultMap scoreWithDetails(long user, @Nonnull Collection<Long> items) {
        ...
        for(Long item : itemSet) {
            double score = userFeatures.ebeMultiply(model.getFeatureWeights()).dotProduct(model.getItemVector(item));
            // Add the predicted offsets to the baseline score
            score += baseline.getIntercept() + baseline.getUserBias(user) + baseline.getItemBias(item);
